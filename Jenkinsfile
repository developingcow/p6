    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script {
                    sshagent(['ssh']) {
                        sh "ssh -o StrictHostKeyChecking=no jenkins@10.0.1.155 'docker pull devopscow/todo:1 && docker stop todo && docker rm todo && docker run -d -p 80:80 devopscow/todo:1'"
                    }
                }
            }
        }
